...
## Audit status: v9 code fixed; scientific-result rebuild pending

Legacy numerical results on this branch were generated by superseded estimators and remain
quarantined. The frozen v8 rerun showed that the prior “all endpoints unavailable” conclusion was partly
caused by stacking locally introduced 80% coverage and count gates that are not requirements in
the cited papers. Those locked, outcome-blind endpoint-local sensitivities recovered subject-level
estimates without selecting thresholds for a desired biological direction. They are now a
historical baseline: v9 fixes exact portal sample accounting, SO boundary indexing, complete 3D
event containment, and stronger provenance contracts, so all dependent scientific artifacts must
be rebuilt before they are called current.
...
[audit corrections](docs/AUDIT_CORRECTIONS_2026-07.md). Rebuild and artifact
rules are in [ARTIFACT_POLICY.md](docs/ARTIFACT_POLICY.md).
...
Compact RESPect/HUP v8 evidence remains as a frozen audited baseline. V9 changes cache values,
schema, and builder digests, so a full current-cache/grid/paired rebuild is required; old v8 bytes
are not silently accepted as current. The historical summaries recovered endpoint-local estimates
while retaining the open inference and validation limitations below.
...
The two publication checks deliberately fail if their current private cache lineage is absent or
...
- `analysis/lecci_faithful_3A.py` — importable Lecci-aligned 3A estimator;
