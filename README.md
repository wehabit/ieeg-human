...
## Audit status: v9 rebuilt and publication-validated

Legacy numerical results generated by superseded estimators remain quarantined. The current v9
results were rebuilt from validated neutral caches with the locked, outcome-blind
`overlap11_endpoint_local` profile. That profile avoids stacking one global 80% support rule over
endpoints with different data requirements; 70%, 75%, 80%, and 90% remain reported sensitivities,
not paper-derived qualification boundaries.
...
does not reach the default cohort minimum of five. HUP has 20/14/16 available 3A records,
6/5/15 available 3B records, and 7 descriptive 3D records among 24 analyzed participants; HUP116
is one explicit structured skip. These are hash-pinned v9 results. Availability is not hypothesis
support: 3B and 3D inference is disabled, 3A is heterogeneous, and none of the measures is
LC-specific. See the authoritative
[v9 QC-sensitivity results](docs/QC_SENSITIVITY_RESULTS_2026-07.md), proof-backed
