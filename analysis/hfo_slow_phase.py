"""
HFO-phase - SO-phase of EXPERT-MARKED ripples & fast-ripples  [ds003498, Zurich, 2 kHz].
...
physiological ones. Complementary to Ripple-coupling (SOZ-excluded, IED-masked, envelope-based).
"""
import os, glob, re
...
OUT = os.path.join(ROOT, "outputs", "hfo_slow_phase")
LINE = 50.0
SO_BAND = (0.5, 1.25)
...
def main():
    ...
    print("\n[HFO-phase] SO-phase clustering (Rayleigh):")
    ...
    for j, reg in enumerate(REGIONS):
        for i, (et, col) in enumerate([("ripple", "tab:blue"), ("fr", "tab:red")]):
            ...
            if len(ph) >= 5:
                ...
                ax.set_title(f"{reg} {et}\n(n={len(ph)})", fontsize=9)
    fig.suptitle("HFO-phase (Zurich ds003498, 2 kHz): SO phase of expert-marked ripples (blue) & "
                 "fast-ripples (red)\nblack line = preferred phase; clustering = SO organizes the events",
                 fontsize=11)
    plt.tight_layout()
    for e in ("png", "svg"):
        fig.savefig(os.path.join(OUT, f"ripple_so_phase.{e}"), dpi=150)
    print(f"\n[HFO-phase] figures in {OUT}")
