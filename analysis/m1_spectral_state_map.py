"""
M1 - Spectral state map (the anchor result, tests H1).
...
Usage:
  python analysis/m1_spectral_state_map.py --max-pt 12 --max-clips 4
  python analysis/m1_spectral_state_map.py --all          # every patient/clip (~GBs)
"""
...
OUT = os.path.join(atlas.ROOT, "outputs", "m1_spectral_state_map")
...
def main():
    ...
    if not args.all:
        pts = pts[:args.max_pt]
    print(f"[M1] {len(pts)} patients; groups={sorted(roi.roi_group.unique())}")
    ...
    print(f"[M1] wrote {len(df)} channel-state rows")
