"""
M4 - Regional gradient (human analogue of the mouse `spike_50hz_interpretation` figure:
...
already-computed M1 (slow-band power) and M2 (slow->spindle MI_z) tables — no re-download.

Outputs (outputs/m4_region_gradient/):
  m4_region_gradient.png  - N3 slow power and coupling by region, per-patient + bootstrap CI
"""
import os
...
OUT = os.path.join(atlas.ROOT, "outputs", "m4_region_gradient")
...
def main():
    ...
    m1 = pd.read_csv(M1); m2 = pd.read_csv(M2)
    ...
    fig.suptitle("M4 (DESCRIPTIVE — pairwise MI_z differences are n.s.): N3 slow-power & "
                 "coupling by MTL region")
    plt.tight_layout()
    for ext in ("png", "svg"):
        plt.savefig(os.path.join(OUT, f"m4_region_gradient.{ext}"), dpi=150)
    ...
    for r in REGIONS:
        print(f"  {r:20s} slow={np.mean(slow[r]) if len(slow[r]) else float('nan'):.3f} "
              ...
              f"(n={len(miz[r])})")
    print(f"[M4] figure + pairwise csv in {OUT}")
