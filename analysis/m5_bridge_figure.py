"""
M5 - Mouse<->Human bridge figure (the one-pager / Westover-pitch slide).
...
OUT = os.path.join(atlas.ROOT, "outputs", "m5_bridge")
...
HUMAN_STATE = os.path.join(atlas.ROOT, "outputs/m2_slow_spindle_pac/m2_pac_by_state.png")
...
def main():
    ...
    axd.text(0.02, 0.80,
             ...
             "  slow-oscillation -> spindle nesting (M1 p=2.9e-6; M2 MI_z p=0.030)\n"
             "  and -> ripple nesting on 1 kHz data (M6 p=0.0015).\n\n"
    ...
    fig.suptitle("M5 — Mouse → Human bridge: from theta engagement to the NREM slow-wave "
                 "coordination state (candidate substrate; clearance not measured)",
                 fontsize=12, y=0.995)
    for ext in ("png", "svg"):
        fig.savefig(os.path.join(OUT, f"m5_bridge.{ext}"), dpi=150, bbox_inches="tight")
    print(f"[M5] wrote bridge figure to {OUT}")
