"""
M6 - Slow-oscillation -> ripple (and spindle) coupling  [Phase-1b, 1 kHz data].
...
that was M1/M2), extended to the ripple band.
...
Outputs (outputs/m6_so_ripple_coupling/):
  m6_comodulogram.png     - SO-phase x amplitude-freq (8-200 Hz): spindle AND ripple hotspots
  m6_coupling_by_region.png - SO->ripple and SO->spindle MI_z by MTL region
...
OUT = os.path.join(ROOT, "outputs", "m6_so_ripple_coupling")
LINE = 50.0
SO_BAND = (0.5, 1.25)
SPINDLE = (11, 16)
RIPPLE = (80, 120)
...
def main():
    ...
    print(f"[M6] {len(df)} channels; regions={df.region.value_counts().to_dict()}")
    ...
    for key in ["ripple", "spindle"]:
        ...
        if len(per_sub) >= 5:
            ...
            print(f"[M6] SO->{key}: mean MI_z={per_sub.mean():.3f} across {len(per_sub)} subjects, "
    ...
    fig.suptitle("M6: NREM slow-oscillation coordinates spindles AND ripples in human MTL "
                 "(1 kHz, IED/SOZ-cleaned)")
    plt.tight_layout()
    for e in ("png", "svg"):
        fig.savefig(os.path.join(OUT, f"m6_coupling_by_region.{e}"), dpi=150)
    ...
    if len(best):
        ...
        plt.title(f"M6 comodulogram — {r0['channel']} ({r0['region']}, {r0['sub']})\n"
                  "NREM slow oscillation couples both spindle and ripple bands")
        plt.tight_layout()
        for e in ("png", "svg"):
            plt.savefig(os.path.join(OUT, f"m6_comodulogram.{e}"), dpi=150)
    print(f"[M6] figures in {OUT}")
