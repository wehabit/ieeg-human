"""
Nesting-timecourse - Temporal (time-domain) figures of the NREM nesting, on the 1 kHz Ripple-coupling data.

The coupling modules (Spindle-coupling/Ripple-coupling) are frequency-domain. This one shows the phenomenon in TIME:
...
Reuses the Ripple-coupling loader (bipolar, SOZ-dropped, IED-masked, 50 Hz notched).
"""
import os, glob
import numpy as np
...
import slow_ripple_coupling as m6

OUT = os.path.join(m6.ROOT, "outputs", "nesting_timecourse")
SF = 1000.0
...
def main():
    ...
    print(f"[Nesting-timecourse] {n_ev} SO troughs pooled; best channel {best['label']} "
    ...
    plt.tight_layout()
    for e in ("png", "svg"):
        fig.savefig(os.path.join(OUT, f"so_triggered_nesting.{e}"), dpi=150)
    ...
    plt.tight_layout()
    for e in ("png", "svg"):
        plt.savefig(os.path.join(OUT, f"so_triggered_tf.{e}"), dpi=150)
    print(f"[Nesting-timecourse] figures in {OUT}")
