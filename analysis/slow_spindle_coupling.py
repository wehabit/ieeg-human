"""
Spindle-coupling - Slow-oscillation -> spindle phase-amplitude coupling (tests H2, H3).
...
Usage:
  python analysis/slow_spindle_coupling.py --max-pt 60 --max-clips 8
"""
...
NBINS = 18
N_SURR = 120
OUT = os.path.join(atlas.ROOT, "outputs", "slow_spindle_coupling")
...
def main():
    ...
    print(f"[Spindle-coupling] {len(pts)} patients; phase {PHASE_BAND} Hz -> amp {AMP_BAND} Hz")
    ...
    print(f"[Spindle-coupling] wrote {len(df)} channel-state PAC rows")
    ...
    if len(n3):
        ...
        ax[1].set_title("Phase-preference/H3: preferred SO phase of\nspindle power (N3, mesiotemporal)")
    plt.tight_layout()
    for ext in ("png", "svg"):
        plt.savefig(os.path.join(OUT, f"spindle_coupling_by_state.{ext}"), dpi=150)
    ...
    print(f"\n[Spindle-coupling] figure + csv in {OUT}")
