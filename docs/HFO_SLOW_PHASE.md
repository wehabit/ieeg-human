...
**Why muddier than Ripple-coupling:** these are **HFO markings for epilepsy localization**, so a substantial
fraction are **pathological** ripples, which are known to couple poorly to sleep rhythms. Ripple-coupling (same
...
- **Ripple-coupling** = physiological, cleaned, envelope-based → clean positive (SO→ripple p=0.0015).
- **HFO-phase** = all marked HFOs incl. pathological, event-based, independent cohort, adds fast-ripples →
...
- A physiological-vs-pathological control that strengthens Ripple-coupling's interpretation.

## Caveats
- HFO markings mix physiological + pathological; no per-channel SOZ flag here to exclude the latter
  (unlike Ripple-coupling). The muddiness is expected, not a null of the mechanism.
...
## Outputs
- `outputs/hfo_slow_phase/so_phase_stats.csv`
- `outputs/hfo_slow_phase/ripple_so_phase.{png,svg}`
