# HUP165 — one selected candidate interval (iEEG.org), polarity-robust SO coupling

> **WITHDRAWN LEGACY RESULT — DO NOT CITE AS CURRENT LC-PROXY EVIDENCE.** This page was
> generated by a separate historical nesting pipeline outside the current v9 provenance, verified
...
> V9 recovers descriptive 3D records but keeps inference disabled; this historical single-subject
> contact analysis cannot replace the locked grid.

This historical analysis used a **continuous selected interval**, unlike the clip cohorts
(atlas/Falach/Zurich). `hup165_night_coupling.py` runs the same polarity-robust, cross-validated coupling
...
| Band | selected-interval cross-val modulation [95% CI over channels] |
|---|---|
