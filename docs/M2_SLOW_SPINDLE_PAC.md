# M2 — Slow-oscillation → Spindle Coupling (tests H2, H3)
...
This spindle-coupling result is **independently confirmed on a separate cohort in M6** (SO→spindle
...
This is the empirical core of the **M5 bridge argument**: the native coordinated state is a
...
- **Depth / variance.** 60 patients scanned, up to 20 clips/state; entorhinal ROI is the thinnest
  ...
  The decisive corroboration is M6's independent-cohort replication (p = 6×10⁻⁵), not more clips.
...
## Outputs
- `outputs/m2_slow_spindle_pac/channel_state_pac.csv` — per channel × state: mi, mi_z, pref_phase.
- `outputs/m2_slow_spindle_pac/group_state_pac.csv` — group × state MI + MI_z.
- `outputs/m2_slow_spindle_pac/m2_pac_by_state.{png,svg}`.

## Next
- **M4** regional gradient (hippocampal vs entorhinal vs neocortex) with proper stats — the human
  dHPC-vs-LEC analogue.
- **M5** mouse↔human bridge figure for the one-pager.
