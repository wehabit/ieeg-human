# M6 — Slow-oscillation → Ripple (and Spindle) Coupling  [Phase-1b, 1 kHz]

**Question:** the mouse study showed CA1 sharp-wave **ripples**; the 204 Hz atlas (M1–M5) could
not touch ripples (Nyquist 102 Hz). Do human NREM slow oscillations organize **ripples** (80–120 Hz)
in the medial temporal lobe — and does the spindle coupling from M2 replicate on cleaner data?
...
(state contrast was M1/M2's job), extended to the ripple band.
...
- **SO → spindle (11–16 Hz): MI_z = 2.15, 15/15 subjects, p = 6.1e-5.** This **independently
  confirms M2**. The coupling that was real-but-noisy at 204 Hz (p = 0.030) is rock-solid on this
  cleaner, IED-removed, higher-rate dataset.
- Both couplings are present across **all four MTL regions** (hippocampal, entorhinal,
  parahippocampal, amygdala); entorhinal — thin and noisy in M2 — is robust here.
...
- **Surrogate-corrected MI_z** (120 circular shifts), same discipline as M2.
...
M6 completes the human coordination picture the Gyuri pointer set implied:

| Mouse figure | Human module | Band |
|---|---|---|
| theta STA / PLV | M2 + comodulogram | spindle (11–16 Hz) |
| **CA1 ripples** | **M6** | **ripple (80–120 Hz)** |
| dHPC vs LEC | M4 | region gradient |
...
structure; the design implication (M5) stands and is now stronger.

## Caveats

- Short NREM-dominant segments (~1–4 min), not full nights → no within-subject state contrast here
  (that is M1/M2). Full-night dynamics (SO→spindle→ripple temporal sequences) still need continuous
  recordings.
...
## Outputs
- `outputs/m6_so_ripple_coupling/channel_coupling.csv`
- `outputs/m6_so_ripple_coupling/m6_coupling_by_region.{png,svg}`
- `outputs/m6_so_ripple_coupling/m6_comodulogram.{png,svg}`
