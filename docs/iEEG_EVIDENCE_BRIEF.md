...
*M1 and M2 are the same atlas cohort (the slow rhythm, then its coupling). M6 is a second,
independent cohort. M8 (below) is a third, supporting cohort.*

### M1 — the slow rhythm is present and state-graded
...
### M2 — the slow oscillation organizes spindles (NREM-specific)
...
### M6 — it also organizes ripples, and independently replicates the spindle result
...
- **SO → spindle: p = 6×10⁻⁵** (15/15 subjects) — an **independent replication of M2** on cleaner
...
*(A third cohort, M8 (Zurich, 2 kHz, expert-marked HFOs), adds fast-ripples and shows the coupling
tracks physiological rather than pathological events — supporting detail, not part of the core
spine.)*

---

## Mouse → Human bridge (figure: `m5_bridge`)
...
| **Human (this work)** | flat coupling | **slow→spindle (M1/M2) + slow→ripple (M6)** | coupling drops |
...
- **M2's effect is modest and between-patient-variable** (p = 0.030); its strength comes from the
  independent M6 replication, not from p alone.
- **Region differences (M4) are descriptive** — pairwise MI_z contrasts are not significant.
...
## Reproducibility
Each claim maps to one script and one figure (`analysis/m1…`, `m2…`, `m6…`, `m5_bridge`); final
